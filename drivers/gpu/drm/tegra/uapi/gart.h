...
static inline int
tegra_bo_gart_map(struct tegra_drm *tegra, struct tegra_bo *bo)
{
	int ret;
	...
	ret = tegra_bo_gart_map_locked(tegra, bo, true);
...
static inline void
tegra_bo_gart_unmap(struct tegra_drm *tegra, struct tegra_bo *bo)
{
	if (!IS_ENABLED(CONFIG_TEGRA_IOMMU_GART) || !tegra->has_gart)
		return;
