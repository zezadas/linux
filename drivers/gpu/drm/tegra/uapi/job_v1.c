...
tegra_drm_check_submit(struct drm_tegra_submit *submit)
{
	if (submit->num_relocs > DRM_TEGRA_BO_TABLE_MAX_ENTRIES_NUM) {
		DRM_ERROR("invalid num_relocs: %u\n", submit->num_relocs);
		return -EINVAL;
	}
...
tegra_drm_unprepare_job(struct tegra_drm_job *job)
{
	...
	list_for_each_entry(drm_client, &job->tegra->clients, list) {
		...
		if (pipes & drm_client->pipe) {
			...
			if (err)
				DRM_DEV_ERROR(drm_client->base.dev,
...
static void tegra_drm_free_job_v1(struct tegra_drm_job *job)
{
	struct tegra_drm_job_v1 *job_v1 = to_tegra_drm_job_v1(job);
		       ...
		       struct tegra_drm_job **ret_job)
{
	...
	syncpt = host1x_syncpt_request(host, drm->dev);
	if (IS_ERR(syncpt)) {
		err = PTR_ERR(syncpt);
				 ...
				 u32 **cmdstream)
{
	...
	*ptr++ = host1x_opcode_setclass(job_v1->host1x_class, 0, 0);
	...
	for (i = 0; i < submit->num_cmdbufs; i++) {
		...
		if (!gem) {
			DRM_ERROR("failed to find cmdbuf bo handle[%u] %u\n",
				  i, cmdbufs[i].handle);
			err = -EINVAL;
			goto err_unlock;
		}
		...
		if (offset & 3 || offset > gem->size - sizeof(u32)) {
			DRM_ERROR("invalid cmdbuf offset %llu\n", offset);
			err = -EINVAL;
			goto err_unlock;
		}
	...
	for (i = 0, k = 0, bufptr = NULL; i < submit->num_relocs; i++) {
		...
		if (k == submit->num_cmdbufs) {
			DRM_ERROR("invalid reloc[%u] cmdbuf.handle %u\n",
...
patch_reloc:
		...
		if (relocs[i].cmdbuf.offset >= offset ||
			...
			DRM_ERROR("invalid reloc[%u] cmdbuf.offset %u\n",
		...
		*ptr = new_reloc.u_data;
		...
		if (!gem) {
			DRM_ERROR("invalid reloc[%u] target.handle %u\n",
			     ...
			     struct drm_tegra_submit *submit)
{
	...
	if (err) {
		DRM_ERROR("failed to allocate host1x bo: %d\n", err);
		return err;
	}
...
tegra_drm_select_channel(struct tegra_drm *tegra, struct tegra_drm_job *job)
{
	...
	if (hweight64(pipes) > 1) {
		DRM_ERROR("uapi v1 doesn't allow to have more than one class per job, pipes %llu\n",
			  pipes);
		return -EINVAL;
	}
	...
	if (!best_channel) {
		DRM_ERROR("failed to select channel, pipes %llu\n", pipes);
		return -EINVAL;
	}
		       ...
		       struct drm_file *file)
{
	...
	err = drm_sched_job_init(&job->sched_job,
				 ...
				 NULL);
	if (err) {
		DRM_ERROR("failed to prepare job for scheduling: %d\n", err);
		return err;
	}
