			 ...
			 struct drm_tegra_submit_v2 *submit)
{
	...
	if (size && copy_from_user(bo_table, user_bo_table, size)) {
		DRM_ERROR("failed to copy bo_table\n");
		return -EFAULT;
	}

	size = sizeof(u32) * submit->num_cmdstream_words;
	if (copy_from_user(cmdstream, user_cmdstream, size)) {
		DRM_ERROR("failed to copy cmdstream\n");
		return -EFAULT;
	}
			    ...
			    struct drm_file *file)
{
	...
	for (i = 0; i < submit->num_bos; i++) {
		...
		if (!gem) {
			DRM_ERROR("failed to find bo handle[%u] = %u\n",
				  i, bo_table[i].handle);
			return -EINVAL;
		}
...
retry:
	...
	for (i = 0; i < num_bos; i++) {
		...
		if (ret) {
			...
			if (ret == -EALREADY)
				DRM_ERROR("bo table has duplicates\n");
			...
			struct drm_tegra_submit_v2 *submit)
{
	...
	for (i = 0; i < job->num_bos; i++) {
		...
		if (bo_table[i].flags & DRM_TEGRA_BO_TABLE_WRITE) {
			...
			err = reservation_object_reserve_shared(resv, 1);
			if (err)
				goto err_put_fences;

			fences[i].excl = reservation_object_get_excl_rcu(resv);
			fences[i].num_shared = 0;
		}
	}
