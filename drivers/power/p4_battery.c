...
static int p3_cable_status_update(struct battery_data *battery, int status)
{
	...
	pr_debug("%s:Update cable status\n", __func__);
	...
	case CHARGER_DISCHARGE:
		...
		break;
	default:
		pr_info("%s:Not supported status\n", __func__);
		ret = -EINVAL;
	}
	...
	struct p3_battery_platform_data *pdata)
{
	...
	if (val >= 0) {
		pdata->charger.enable_line = val;
	} else {
		return val;
	}

	val = of_get_named_gpio(of_node, "connect-line", 0);
	if (val >= 0) {
		pdata->charger.connect_line = val;
	} else {
		return val;
	}

	val = of_get_named_gpio(of_node, "fullcharge-line", 0);
	if (val >= 0) {
		pdata->charger.fullcharge_line = val;
	} else {
		return val;
	}

	val = of_get_named_gpio(of_node, "currentset-line", 0);
	if (val >= 0) {
		pdata->charger.currentset_line = val;
	} else {
		return val;
	}
	...
	if (!of_property_read_u32(of_node, "temp-high-recovery", &val))
		pdata->temp_high_recovery = val;

	if (!of_property_read_u32(of_node, "temp-low-recovery", &val))
		pdata->temp_low_recovery = val;

	if (!of_property_read_u32(of_node, "temp-low-threshold", &val))
		pdata->temp_low_threshold = val;

	if (!of_property_read_u32(of_node, "charge-duration", &val))
		pdata->charge_duration = val;

	if (!of_property_read_u32(of_node, "recharge-duration", &val))
		pdata->recharge_duration = val;
	...
	pr_debug("%s: fullcharge_line=<%d>\n", __func__, pdata->charger.fullcharge_line);
	pr_debug("%s: currentset_line=<%d>\n", __func__, pdata->charger.currentset_line);
	pr_debug("%s: alert_line=<%d>\n", __func__, pdata->charger.alert_line);

	pr_debug("%s: temp_high_threshold=<%d>\n", __func__, pdata->temp_high_threshold);
	pr_debug("%s: temp_high_recovery=<%d>\n", __func__, pdata->temp_high_recovery);
	pr_debug("%s: temp_low_recovery=<%d>\n", __func__, pdata->temp_low_recovery);
	pr_debug("%s: temp_low_threshold=<%d>\n", __func__, pdata->temp_low_threshold);
	pr_debug("%s: charge_duration=<%d>\n", __func__, pdata->charge_duration);
	pr_debug("%s: recharge_duration=<%d>\n", __func__, pdata->recharge_duration);
	pr_debug("%s: recharge_voltage=<%d>\n", __func__, pdata->recharge_voltage);
...
static int p3_bat_remove(struct platform_device *pdev)
{
	struct battery_data *battery = platform_get_drvdata(pdev);
...
static const struct of_device_id p3_bat_of_ids[] = {
...
static struct platform_driver p3_bat_driver = {
	.driver = {
		.name = "p3-battery",
		.of_match_table = p3_bat_of_ids,
		.owner  = THIS_MODULE,
		.pm = &p3_battery_pm_ops,
	},
	.probe		= p3_bat_probe,
	.remove		= p3_bat_remove,
};
