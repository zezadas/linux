...
void fg_periodic_read(void)
{
	...
	pr_info("[MAX17042] %d/%d/%d %02d:%02d,%04Xh\n",
		tm.tm_mday, (tm.tm_mon + 1), (tm.tm_year + 2000),
		tm.tm_hour, tm.tm_min, accum);
}
...
static int parse_dt(struct max17042_chip *chip, struct device_node *of_node)
{
	...
	if (!of_property_read_u32(of_node, "sdi-vfcapacity", &val))
		pdata->sdi_vfcapacity = val;

	if (!of_property_read_u32(of_node, "atl-capacity", &val))
		pdata->atl_capacity = val;
