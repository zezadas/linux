...
#ifdef CONFIG_OF
static int stmpe811_parse_dt(struct device_node *np, int *adc_int_gpio)
{
	u32 val;
	int ret;

	val = of_get_named_gpio(np, "adc-interrupt-gpio", 0);
	if (val >= 0) {
		*adc_int_gpio = val;
		ret = 0;
	}

	pr_debug("%s: adc-interrupt-gpio=<%d>\n", __func__, *adc_int_gpio);

	return ret;
}
#else
static int stmpe811_parse_dt(struct device_node *, int *)
{
	return -EINVAL;
}
#endif
...
static int stmpe811_adc_i2c_probe(struct i2c_client *client,  const struct i2c_device_id *id)
{
	...
	u8 data[2];
	u16 w_data;
	int adc_int_gpio = -EINVAL;

	printk("stmpe811_adc_i2c_probe!!!\n");

	if (stmpe811_parse_dt(np, &adc_int_gpio)) {
		pr_err("%s: error parsing device tree node\n", __func__);
		return -EINVAL;
	}

#if !defined(CONFIG_MACH_SAMSUNG_P5)
	if (gpio_request(adc_int_gpio, "GPIO_ADC_INT") < 0) {
		pr_info("%s: error requesting GPIO_ADC_INT\n", __func__);
		return -EINVAL;
	}
	gpio_direction_input(adc_int_gpio);
#endif
...
#ifdef CONFIG_OF
static const struct of_device_id stmpe811_dt_match[] = {
	{ .compatible = "samsung,stmpe811" },
	{ },
};
MODULE_DEVICE_TABLE(of, stmpe811_dt_match);
#endif

static struct i2c_driver stmpe811_adc_i2c_driver = {
	.driver = {
		.name = "stmpe811",
		.of_match_table = of_match_ptr(stmpe811_dt_match),
		.owner = THIS_MODULE,
	},
