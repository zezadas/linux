...
#ifdef CONFIG_OF
static struct bcm4751_power_platform_data* bcm4751_parse_dt(struct platform_device *pdev)
{
	struct bcm4751_power_platform_data *pdata;
...
static int bcm4751_power_probe(struct platform_device *pdev)
{
	...
	if (device_create_file(&pdev->dev, &dev_attr_nrst) < 0) {
	...
	if (device_create_file(&pdev->dev, &dev_attr_pwr_en) < 0) {
		printk(KERN_ERR "%s: failed to create device file!(%s)!\n",
			__func__, dev_attr_pwr_en.attr.name);
		device_remove_file(&pdev->dev, &dev_attr_nrst);
		ret = -EINVAL;
		goto fail;
	}
...
static int bcm4751_power_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_pwr_en);
	device_remove_file(&pdev->dev, &dev_attr_nrst);
