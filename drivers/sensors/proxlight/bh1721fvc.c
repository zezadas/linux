...
static int bh1721fvc_init_gpio(struct i2c_client *client, struct bh1721fvc_data *bh1721fvc)
{
	int err;

	err = devm_gpio_request(&client->dev, bh1721fvc->gpio, "LIGHT_SENSOR_DVI");
	if (err) {
		pr_err("%s: error requesting LIGHT_SENSOR_DVI gpio\n", __func__);
		return err;
	}

	err = gpio_direction_output(bh1721fvc->gpio, 1);
	if (err) {
		pr_err("%s: error gpio direction output for LIGHT_SENSOR_DVI\n", __func__);
		return err;
	}
...
#ifdef CONFIG_OF
static int bh1721fvc_parse_dt(struct i2c_client *client, struct bh1721fvc_data *bh1721fvc)
{
	...
	const struct i2c_device_id *id)
{
	...
	struct bh1721fvc_data *bh1721fvc;
	struct input_dev *input_dev;
	struct bh1721fvc_platform_data *pdata = NULL;
	...
	if (client->dev.platform_data) {
		...
		pdata = kzalloc(sizeof(struct bh1721fvc_platform_data), GFP_KERNEL);
	}
	...
	err = bh1721fvc_parse_dt(client, bh1721fvc);
	if (err) {
		pr_err("%s: error parsing device tree\n", __func__);
		goto done;
	}

	err = bh1721fvc_init_gpio(client, bh1721fvc);
	if (err) {
		pr_err("%s: error initializing gpio\n", __func__);
		goto done;
	}
...
static int  bh1721fvc_remove(struct i2c_client *client)
{
	...
	devm_gpio_free(&client->dev, bh1721fvc->gpio);
...
#ifdef CONFIG_OF
static const struct of_device_id bh1721fvc_dt_match[] = {
	{ .compatible = "samsung_p3,bh1721fvc" },
	{ },
};
MODULE_DEVICE_TABLE(of, bh1721fvc_dt_match);
#endif
...
static struct i2c_driver bh1721fvc_driver = {
	.driver = {
		...
		.of_match_table = of_match_ptr(bh1721fvc_dt_match),
		.owner	= THIS_MODULE,
		.pm	= &bh1721fvc_pm_ops,
	},
