...
static irqreturn_t p4wifi_jack_detect_irq_thread(int irq, void *dev_id)
{
	...
	__pm_wakeup_event(&info->det_wakeup_source, WAKE_LOCK_TIME);

	// Debounce the interrupt
	while (time_left > 0) {
		unsigned int val = gpiod_get_value(pdata->detect_gpio) ^
				   npolarity;
...
void p4wifi_jack_buttons_work(struct work_struct *work)
{
	...
	adc = p4wifi_jack_read_adc(info);
	for (i = 0; i < pdata->num_buttons_zones; i++) {
		if (adc >= btn_zones[i].adc_low && adc <=
		    btn_zones[i].adc_high) {
...
static int p4wifi_jack_probe(struct platform_device *pdev)
{
	...
	pdata->ear_micbias_gpio = devm_gpiod_get(dev, "ear-micbias",
						 GPIOD_OUT_HIGH);
	...
	if (IS_ERR(info->adc_channel)) {
		...
		if (err != -EPROBE_DEFER)
			dev_err(dev, "failed to get headset-jack-detect ADC"
				"channel\n");
		return err;
	}
	...
	wakeup_source_init(&info->det_wakeup_source, "headset-jack-detect");
...
err_create_wq_failed:
	wakeup_source_remove(&info->det_wakeup_source);
