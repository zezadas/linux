...
static int iovmm__proc_show(struct seq_file *m, void *v)
{
...
static int iovmm_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, iovmm__proc_show, NULL);
}

static const struct file_operations iovmm_proc_fops = {
	.open           = iovmm_proc_open,
	.read           = seq_read,
	.llseek         = seq_lseek,
	.release        = seq_release,
};
...
int tegra_iovmm_register(struct tegra_iovmm_device *dev)
{
	...
	if (list_empty(&iovmm_devices)) {
		...
		proc_create("iovmminfo", S_IRUGO, NULL, &iovmm_proc_fops);
	}
