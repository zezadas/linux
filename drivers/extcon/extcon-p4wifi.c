...
static int p4wifi_usb_init_path(struct p4wifi_usb_data *data){
	...
	/*
	 * usb_sel2 gpio was gotten with flag GPIOD_ASIS so that
	 * the initial boot value of usb_sel2 can be read here.
	 * After reading the value usb_sel2 is set to direction output.
	 */
...
static int p4wifi_usb_init_adc_channel(struct platform_device *pdev,
	struct p4wifi_usb_data *data)
{
	...
	chan = iio_channel_get(NULL, "stmpe-adc-ch6");
	if (IS_ERR(chan)) {
		ret = PTR_ERR(chan);
		dev_err(&pdev->dev, "failed to get stmpe-adc-ch6 (%d)\n",
			ret);
		return ret;
	}
	data->adc_channel = chan;
...
static int p4wifi_usb_probe(struct platform_device *pdev)
{
	...
	ret = p4wifi_usb_init_adc_channel(pdev, data);
	if (ret) {
		ret = -EPROBE_DEFER;
		goto err;
	}
...
int p4wifi_usb_remove(struct platform_device *pdev)
{
	return 0;
}
