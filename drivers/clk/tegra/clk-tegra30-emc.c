...
#define EMC_1X_CLK_ENB			BIT(24)
#define EMC_2X_CLK_ENB			BIT(25)
#define USE_PLLM_UD			BIT(29)

#define CLK_SOURCE_EMC_EMC_2X_CLK_DIVISOR_SHIFT 0
#define CLK_SOURCE_EMC_EMC_2X_CLK_DIVISOR_MASK 0xff
#define CLK_SOURCE_EMC_EMC_2X_CLK_DIVISOR(x) \
	(((x) & CLK_SOURCE_EMC_EMC_2X_CLK_DIVISOR_MASK) << \
	 CLK_SOURCE_EMC_EMC_2X_CLK_DIVISOR_SHIFT)

#define CLK_SOURCE_EMC_EMC_2X_CLK_SRC_SHIFT 30
#define CLK_SOURCE_EMC_EMC_2X_CLK_SRC_MASK 0x3
#define CLK_SOURCE_EMC_EMC_2X_CLK_SRC(x) \
	(((x) & CLK_SOURCE_EMC_EMC_2X_CLK_SRC_MASK) << \
			  ...
			  struct emc_timing *timing)
{
	...
	if (timing->parent_index == EMC_SRC_PLL_M && div == 0) {
		car_value |= USE_PLLM_UD;
	} else {
		car_value &= ~USE_PLLM_UD;
	}

	/*
	 * This value is determined by MC register MC_EMEM_ARB_MISC0_INDEX
	 * at bit MC_EMEM_ARB_MISC0_EMC_SAME_FREQ
	 */
	if (timing->mc_same_freq) {
		car_value |= MC_EMC_SAME_FREQ;
	}
			...
			unsigned long parent_rate)
{
	...
	if (clk_hw_get_rate(hw) == rate) {
		pr_err("%s: hw rate = %lu, rate = %lu\n", __func__,
			clk_hw_get_rate(hw), rate);
		return 0;
	}
	...
	err = emc_set_timing(tegra, timing);
				   ...
				   struct device_node *node)
{
	...
	if (IS_ERR(timing->parent)) {
		pr_err("timing %pOF: failed to get parent clock. (%ld)\n", node,
		       PTR_ERR(timing->parent));
	...
	timing->mc_same_freq = of_property_read_bool(node, "mc-same-freq");
				   ...
				   spinlock_t *lock)
{
	...
	clk_set_parent(clk, clk_hw_get_parent_by_index(&tegra->hw,
		       emc_get_parent(&tegra->hw))->clk);
